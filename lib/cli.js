...
switch (mainOptions.command) {
  ...
  case 'deploy':
    const deployDefinitions = [
      ...
      { name: 'verbose', type: Boolean },
      { name: 'endpoint', alias: 'e', type: String },
      { name: 'containerVersion', alias: 'c', type: Number},
      { name: 'ionicVersion', alias: 'i', type: Number}
    ];
...
function runDeploy() {
  ...
  // }
  if ( deployOptions.containerVersion >= 3 ) {
    runDeploy3GC();
  } else {
    runDeploy2GC();
  }
...
async function runDeploy2GC() {
  const deploy = require('./deploy.js');
  if (getIonicVsn() >= 2) {
    const validPkgJson = deploy.validatePackageJson();
    if (validPkgJson === true) {
      const conn = await utils.setupOrgConn(deployOptions);
      ...
      console.log('Missing attribute(s) from package.json'.error);
      validPkgJson.forEach(att => {
        console.log('"' + att + '"');
      });
      process.exit(0);
    }
...
async function runDeploy3GC() {
  ...
  if (validPkgJson === true) {
    ...
    deploy
      .checkVersions(conn)
      .then(r => {
        const ionicVsn = getIonicVsn();
        switch (true) {
          case (ionicVsn < 2) :
            return deploy.prepDeployAssetsV13GC(deployOptions);
          case (ionicVsn < 5) :
            return deploy.prepDeployAssets3GC(deployOptions);
          default:
            return deploy.prepI8DeployAssets3GC(deployOptions);
        }
      })
      .then(r => {
        console.log('Deploy assets prepped'.info);
        return deploy.deploy3GC(conn);
      })
      .then(r => {
        console.log('DEPLOY SUCCESSFUL'.info);
      })
...
function getIonicVsn() {
  ...
  if (packageJson.devDependencies || packageJson.dependencies) {
    ...
    if (
      ...
      ionicVsn = packageJson.devDependencies['ionic-angular'];
