...
function runShellTask(str){
  ...
  child.stdout.on('data', function(data) {
    ...
    writeToLog(mcBuildLog, 'npm data: ' + data);
    inRcvd = true;
  });
  child.stderr.on('data', function (data) {
    console.log('\n: ' + data);
    writeToLog(mcBuildLog, 'npm ERROR: ' + data);
    inRcvd = true;
  });
  ...
  child.on('close', function (code) {
    ...
    if (code !== 0) {
      ...
      writeToLog(mcBuildLog, 'Boo! NPM process exited with code ' + code);
      q.reject(code);
    } else {
      console.log('YAY! NPM process exited OK.');
      writeToLog(mcBuildLog, 'YAY! NPM process exited OK.');
      q.resolve();
    }
  });
  return q.promise;
...
function installNpmDeps(targetPath){
  var q = Q.defer();
  console.log('Installing project NPM dependancies into ' + targetPath + ' (may take a few moments)...');
  cd(targetPath);
  var complete = false;
  var inRcvd = true;
