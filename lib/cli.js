...
var monitorOptions, serveOptions = {};
...
var optimist = require('optimist').
    usage('Usage: \n' +
      ...
      'mobilecaddy monitor --username <USERNAME> [--loginurl <LOGIN_URL>]').
    ...
    describe('loginurl', 'Set \'monitor\' to login with a specific login URL. Default: \'https://login.salesforce.com\'').
...
async function runMonitor(options) {
  var conn = await setupOrgConn(options);
  conn.streaming.topic("CSUpdates").subscribe(function(message) {
    //console.log('Event : ' + JSON.stringify(message));
    var cs = message.sobject;
    if ((message.event.type == 'created' && cs.mobilecaddy1__Session_Type__c != 'Sync - Update')|| message.event.type == 'updated' && cs.mobilecaddy1__Session_Type__c == 'Sync - Update'
      ) {
      cs.mobilecaddy1__MC_Internal_Error_Number__c = (cs.mobilecaddy1__MC_Internal_Error_Number__c) ? cs['mobilecaddy1__MC_Internal_Error_Number__c'].error : "OK ".info;

      if (!cs.mobilecaddy1__Mobile_Table_Name__c ) cs.mobilecaddy1__Mobile_Table_Name__c = '';
      if (cs.mobilecaddy1__Mobile_Table_Name__c == "Connection_Session__mc") cs.mobilecaddy1__Mobile_Table_Name__c = "Conn_Sess__mc";
...
function monitorPrintUpdate(cs) {
  ...
  var succTotal = '0', failTotal = '0';

  // Insert / Updare Success
  var succInsertCnt = (cs.mobilecaddy1__Insert_Successes_Count__c) ? cs.mobilecaddy1__Insert_Successes_Count__c : 0;
  var succUpdateCnt = (cs.mobilecaddy1__Update_Successes_Count__c) ? cs.mobilecaddy1__Update_Successes_Count__c : 0;
  succTotal = (succInsertCnt + succUpdateCnt).toString();

  // Insert Failures
  var failInsertDupeCnt = (cs.mobilecaddy1__Insert_Failure_Duplication_Count__c) ? cs.mobilecaddy1__Insert_Failure_Duplication_Count__c : 0;
  var failIFMFCnt = (cs.mobilecaddy1__Insert_Failure_Match_Failures_Count__c) ? cs.mobilecaddy1__Insert_Failure_Match_Failures_Count__c : 0;
  var failInsertCnt = (cs.mobilecaddy1__Insert_Failures_Count__c) ? cs.mobilecaddy1__Insert_Failures_Count__c : 0;
  // Update Failures
  var failUFMFCnt = (cs.mobilecaddy1__Update_Failure_Match_Failures_Count__c) ? cs.mobilecaddy1__Update_Failure_Match_Failures_Count__c : 0;
  var failUpCnt = (cs.mobilecaddy1__Update_Failures_Count__c) ? cs.mobilecaddy1__Update_Failures_Count__c : 0;
  failTotal = (failInsertDupeCnt + failIFMFCnt + failInsertCnt + failUFMFCnt + failUpCnt );
  if (failTotal > 0) {
  ...
  if (args[1] && args[2]) {
    ...
    if (args[3]) {
      ...
      if (fs.existsSync(args[3])) {
        ...
        path = (args[3].charAt(args[3].length - 1) == "/") ? args[3] : args[3] + "/";
    ...
    getTemplatesList().then(function(knownTemplates){
      ...
      if (template) {
        ...
        fetchArchive(targetPath, appName, template).then(function(){
          ...
          if (res != "no-bower.json") console.log('Bower dependencies installed'.info);
...
function runServe(){
  ...
  if ( getIonicVsn() >= 2 ) {
    ...
    let cmd = spawn('ionic', ['serve', '--p', '3030'], { shell: true, stdio: 'inherit' });
    cmd.on('exit', function(code){
      console.log('exited ' + code);
    });
  } else {
    var child3 = exec('grunt serve ' + scrub + ' ' + local + ' ' + rec, {async:true});
...
function runDesktopBuild(){
  ...
  if (args[1]) {
    ...
    var deployEnv = _.find(pjson.deployEnvs, function(el){return el.name == deployEnvName;})
      if (deployEnv) {
        console.log('Starting .exe build for "'.info + deployEnvName.data + '" with loginEndpoint "'.info + deployEnv.loginEndpoint.data + '"');
        writeToLog(mcBuildLog,'\n - - - - - - - - - - - - - - -');
        writeToLog(mcBuildLog, '>>> Starting .exe build for "' + deployEnvName + '" with loginEndpoint ' + deployEnv.loginEndpoint);
        // Copy package.json to temp file
        fse.copy(packageFile, tmpPackageFile).then(() => {
          // Replace variables
          ...
          // Run build command
          return runShellTask("npm run dist");
        }).then(() => {
          ...
          writeToLog(mcBuildLog, '✔ .exe Build complete');
          process.exit(1);
        }).catch(err => {
          // Move temp back to package.json
          console.error('❌ .exe Build failed'.error);
          writeToLog(mcBuildLog, '❌ .exe Build failed');
          // Move temp back to package.json
          fse.copy(tmpPackageFile, packageFile, { replace: true },function (err2) {
            if (err2) {
    ...
    console.error('You need more arguments, man. Supply a deploy-envs.name!'.error);
    //optimist.showHelp();
    process.exit(0);
  }
}
...
function runDeploy(){
  ...
  let bundleName = packageJson.name + '__' + appVsnSfx + '.zip';
  let mcTmpDir = 'mc-tmp';
  replaceHtmlTitles(packageJson.name).then(function() {
    // Create tmp dir if needed
    if (!fs.existsSync(mcTmpDir)){
      return Promise.resolve();
    } else {
      return runShellTask("rm -r " + mcTmpDir);
    }
  }).then(function(result){
    fs.mkdirSync(mcTmpDir);
    console.log("Create temp files...");
    return runShellTask("ionic build && cp -r www/* "+ mcTmpDir + "/ && rm -r " + mcTmpDir + "/assets/js/");
  }).then(function(result){
    console.log("Cleaning temp files...");
    if (fs.existsSync(mcTmpDir + "/mock")){
      return runShellTask("rm -r " + mcTmpDir + "/mock");
    } else {
      return Promise.resolve();
    }
  }).then(function() {
    ...
    return uppercaseFontRefsinCSS();
  }).then(function() {
    if (fs.existsSync(bundleName)){
      return runShellTask("rm " + bundleName);
    } else {
      return Promise.resolve();
    }
  }).then(function() {
    ...
    zipdir(mcTmpDir, { saveTo: bundleName }, function (err, buffer) {
      // `buffer` is the buffer of the zipped file
      // And the buffer was saved to bundleName
      createApexTemplates().then(function(result){
        console.log("DONE");
      }).catch(function(e){
...
function createApexTemplates(){
  return new Promise(function(resolve, reject) {
    const replace = require('replace-in-file');
    let mcTmpDir = 'mc-tmp';
    const appVsnSfx = packageJson.version.replace(/\./g, '');
    const myAppResource = packageJson.name + '__' + appVsnSfx;
    const myCacheResource = packageJson.name + 'Cache__' + appVsnSfx;
    // Create tmp cache and startpages
    runShellTask("cp apex-templates/startpage-template.apex " + mcTmpDir + "/" + myAppResource + '.apex').then(function(result){
      return runShellTask("cp apex-templates/cachepage-template.apex " + mcTmpDir + "/" + myCacheResource + '.apex')
    }).then(function(result){
...
function runDesktopMSIBuild(){
  ...
  if (args[1]) {
    ...
    var deployEnv = _.find(pjson.deployEnvs, function(el){return el.name == deployEnvName;})
      if (deployEnv) {
        console.log('Starting .exe --> MSI conversion for "'.info + deployEnvName.data + '", v' + pjson.version.data);
        writeToLog(mcBuildLog,'\n - - - - - - - - - - - - - - -');
        writeToLog(mcBuildLog, '>>> Starting .exe --> MSI conversion for ' + deployEnvName + '", v' + pjson.version);
        // Copy package.json to temp file
        fse.copy(exemsiConfFile, tmpExemsiConfFile).then(() => {
          // Replace variables
          var replace = require("replace");
          replace({
            regex: "<DEPLOY-ENV>",
          ...
          // Run build command
          return runShellTask("\"C:\\Program Files (x86)\\MSI Wrapper\\MsiWrapperBatch.exe\" config=\"build\\exemsi.xml\"");
        }).then(() => {
          // Move temp back to package.json
          ...
          writeToLog(mcBuildLog, '✔ Conversion complete');
          process.exit(1);
        }).catch(err => {
          // Move temp back to exemsi conf
          console.error('❌ MSI conversion failed'.error);
          writeToLog(mcBuildLog, '❌ MSI conversion failed');
          // Move temp back to package.json
          fse.copy(tmpExemsiConfFile, exemsiConfFile, { replace: true },function (err2) {
            if (err2) {
              console.error("Err2 " + err2);
            }
            // Remove temp files
...
function runShellTask(str){
  // console.log("runShellTask", str);
  var q = Q.defer();
  var child = exec(str, {async:true});
  child.stdout.on('data', function(data) {
    console.log(data);
    // writeToLog(mcBuildLog,  data);
    inRcvd = true;
  });
  child.stderr.on('data', function (data) {
    console.log('\n: ' + data);
    writeToLog(mcBuildLog, 'ERROR: ' + data);
    inRcvd = true;
  });
  ...
  child.on('close', function (code) {
    ...
    if (code !== 0) {
      ...
      writeToLog(mcBuildLog, 'Boo! process exited with code ' + code);
      q.reject(code);
    } else {
      // console.log('YAY! process exited OK.');
      writeToLog(mcBuildLog, 'YAY! process exited OK.');
      q.resolve();
    }
  });
  return q.promise;
...
function getIonicVsn() {
  ...
  if (packageJson.devDependencies || packageJson.dependencies) {
    if (packageJson.dependencies && packageJson.dependencies['ionic-angular']) ionicVsn = packageJson.dependencies['ionic-angular'];

    if (packageJson.dependencies && packageJson.dependencies['ionic-sdk']) ionicVsn = packageJson.dependencies['ionic-sdk'];

    if (packageJson.devDependencies['ionic-sdk']) ionicVsn = packageJson.devDependencies['ionic-sdk'];
...
function installNpmDeps(targetPath){
  return new Promise(function(resolve, reject) {
    console.log('Installing project NPM dependencies into ' + targetPath + ' (may take a few moments)...');
    cd(targetPath);
    var complete = false;
    var inRcvd = true;
...
function installBowerDeps(targetPath){
  return new Promise(function(resolve, reject) {
    if (fs.existsSync("bower.json")) {
      console.log('Installing project bower dependencies (may take a few moments)...');
...
function replaceNamePlaceholders(appName) {
  return new Promise(function(resolve, reject) {
    ...
    replace(options).then(function(result){
      return replaceHtmlTitles(appName);
    }).then(function(result){
      resolve();
    // index.html
    }).catch(function(e){
...
function replaceHtmlTitles(appName) {
  return new Promise(function(resolve, reject) {
    ...
    let options = {
      files: [
        'src/index.html',
        'apex-templates/startpage-template.apex'
      ],
      from: /\<title\>.*\<\/title\>/g,
...
function maybeShowNodeWarning(){
  ...
  if ( getIonicVsn() > 2 &&  cmp < 0 ) {
    console.log("You need to be running node 8.5.0+ to use Angular v2+ / Ionic v2+".error);
    process.exit(0);
  }
}
...
async function setupOrgConn(options) {
  return new Promise(function(resolve, reject) {
    const loginUrl = (options.loginurl) ? options.loginurl : 'https://login.salesforce.com';
    var conn = new jsforce.Connection({
      loginUrl: loginUrl
    });

    if (options.username) { // username / password login
      readPassword().then(function(password){
...
function uppercaseFontRefsinCSS(){
  return new Promise(function(resolve, reject) {
    const replace = require('replace-in-file');
    let options = {
      files: 'mc-tmp/build/*.css*',
      from: [/\.woff/g,  /\.ttf/g ,/\?v=.*\"/g],
      to: ['.WOFF', '.TTF', '"'],
    };
    replace(options).then(function(result){
      let options = {
