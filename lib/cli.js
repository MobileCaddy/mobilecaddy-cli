...
function runDesktopBuild(){
  ...
  if (args[1]) {
    ...
    var deployEnv = _.find(pjson.deployEnvs, function(el){return el.name == deployEnvName;})
      if (deployEnv) {
        console.log('Starting MSI build for "'.info + deployEnvName.data + '"" with loginEndpoint "'.info + deployEnv.loginEndpoint.data + '"');
        writeToLog(mcBuildLog, '>>> Starting MSI build for ' + deployEnvName + ' with loginEndpoint ' + deployEnv.loginEndpoint);
        // Copy package.json to temp file
        fse.copy('package.json', tmpPackageFile).then(() => {
          // Replace variables
          ...
          // Run build command
          return runNPMTask("npm run dist");
        }).then(() => {
          ...
          writeToLog(mcBuildLog, '✔ Build complete');
          process.exit(1);
        }).catch(err => {
          // Move temp back to package.json
          fse.copy(tmpPackageFile, packageFile);
          writeToLog(mcBuildLog, '❌ Build failed');
          console.error(err.error)
          process.exit(0)
        });
      } else {
...
function runNPMTask(str){
  console.log("runNPMTask", str);
  var q = Q.defer();
