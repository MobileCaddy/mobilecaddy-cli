...
// JUST A DUMMY IMPLEMENTATION TO DEMONSTRATE WHAT MIGHT BE POSSIBLE
function runCodefow(){
  if (args[1]) {
    switch (args[1]) {
      case "tables":
        getTables().then(function(res){
          console.log("We have " +  res + " localstorage tables");
        });
        break;
      default :
        request({uri: 'http://localhost:3030/codeflow/localstorage/' + args[1], json : true}, function (error, response, body) {
...
function getTables(){
  var q = Q.defer();
  getNumTables().then(function(numTables){
    q.resolve(numTables);
    for (var i = 0; i < numTables; i++) {
      request({uri: 'http://localhost:3030/codeflow/localstorage/key/' + i, json : true}, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        console.log("Table: " + body.key);
      } else {
        q.reject("Failed to get table contents -> " + error);
      }
    });
    }
  });
...
function getNumTables() {
  var q = Q.defer();
  request({uri: 'http://localhost:3030/codeflow/localstorage/length', json : true}, function (error, response, body) {
    if (!error && response.statusCode == 200) {
      q.resolve(body.length);
    } else {
      q.reject("Failed to get number of tables -> " + error);
    }
  });
  return q.promise;
}
