    ...
    if (objs.apps) {
      ...
      if (app) {
        ...
        Object.keys(app).forEach(function(key){
          console.log(" " + key.toLocaleUpperCase() + ": " + app[key].data);
        });

      } else {
        console.log("No info for app names ".error + appName.data);
      }
    } else{
      console.log("No Apps currently saved".info);
    }
  } else {
    console.log("No App defined or found in current directory".error);
  }
  process.exit(0);
}
