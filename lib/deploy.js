  ...
  return new Promise(function(resolve, reject) {
    ...
    utils
      ...
      .then(function(result) {
        ...
        fs.mkdirSync(mcTmpBundleDir);
        console.log('Building project...');
        let ionicbuildFlag = deployOptions.prod ? ' --release --aot false --environment prod --output-hashing all --sourcemaps false --extract-css true --named-chunks false --build-optimizer true --minifyjs=true --minifycss=true --optimizejs=true ' : '';
        return utils.runShellTask(
          'ionic build ' +
            ionicbuildFlag +
            ' && cp -r www/build ' +
