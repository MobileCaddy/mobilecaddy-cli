...
function prepDeployAssets(deployOptions) {
  return new Promise(function(resolve, reject) {
    let bundleName = packageJson.name + '_' + packageJson.sf_app_vsn + '.zip';
    let mcTmpDir = 'mc-tmp';
    utils
      ...
      .then(function(result) {
        ...
        if (fs.existsSync(mcTmpDir + '/mock')) {
          ...
          // TODO remove all www/mock-* dirs too
        } else {
          return Promise.resolve();
        }
      })
  ...
  return new Promise(function(resolve, reject) {
    ...
    utils
      ...
      .then(function(result) {
        ...
        jsFilesSWPageStr = swFileEntryArr.join(',');
        let options = {
