  ...
  return new Promise(function(resolve, reject) {
    ...
    utils
      ...
      .then(function(result) {
        ...
        fs.mkdirSync(mcTmpBundleDir);
        console.log('Building project...');
        let ionicbuildFlag = deployOptions.prod ? ' --release --aot false --environment prod --output-hashing all --sourcemaps --extract-css true --named-chunks false --build-optimizer true --minifyjs=true --minifycss=true --optimizejs=true ' : '';
        return utils.runShellTask(
          'ionic build ' +
            ionicbuildFlag +
            ' && cp -r www/build ' +
...
function prepI8DeployAssets3GC(deployOptions) {
  return new Promise(function(resolve, reject) {
    let bundleName = packageJson.sf_app_name + '_' + packageJson.sf_app_vsn + '_3GC.zip';
    const mcTmpDir = 'mc-build';
    const mcTmpBundleDir = 'appbundle';
    utils.runShellTask('rm -r ' + mcTmpDir + '/' + mcTmpBundleDir)
    // utils.runShellTask('ls')
      .then(function(result) {
        // fs.mkdirSync(mcTmpDir);
        fs.mkdirSync(mcTmpDir + '/' + mcTmpBundleDir);
        console.log('Building project...');
        let ionicbuildFlag = deployOptions.prod ? ' --release --aot false --environment prod --output-hashing all --sourcemaps --extract-css true --named-chunks false --build-optimizer true --minifyjs=true --minifycss=true --optimizejs=true ' : '';
        return utils.runShellTask(
          'ionic build ' + ionicbuildFlag +
          ' && cp -r www/* ' + mcTmpDir + '/' + mcTmpBundleDir +
          ' && rm -r ' + mcTmpDir + '/' + mcTmpBundleDir + '/assets/mock*' +
          // ' && rm ' + mcTmpDir + '/' + mcTmpBundleDir + '.zip' +
          ' && cd ' + mcTmpDir +
          ' && mv ' + mcTmpBundleDir + '/styles.*.css ' + mcTmpBundleDir + '/styles.css ' +
          ' && mv ' + mcTmpBundleDir + '/runtime.*.js ' + mcTmpBundleDir + '/runtime.js ' +
          ' && mv ' + mcTmpBundleDir + '/polyfills.*.js ' + mcTmpBundleDir + '/polyfills.js ' +
          ' && mv ' + mcTmpBundleDir + '/main.*.js ' + mcTmpBundleDir + '/main.js ' +
          ' && cp ../mc-templates/index.html ' + mcTmpBundleDir + '/index.html',
          false
        );
      })
      .then(r => {
      ...
      .then(r => {
        return updateLocalIndex(mcTmpDir + '/' + mcTmpBundleDir);
      })
      .then(r => {
        // bundle the zip
        console.log('Creating bundle...');
  ...
  return new Promise(function(resolve, reject) {
    ...
    utils
      ...
      .then(function(result) {
        ...
        // Add entries for all required image files to cache
        let cacheImgFileEntryArr = [];
        imgFiles.forEach(fileName => {
          cacheImgFileEntryArr.push('\n{!URLFOR($Resource.' +
            myAppResource +
            ", 'assets/imgs/" +
            fileName +
            "')}");
        });
        const imgFilesCachePageStr = cacheImgFileEntryArr.join(' ');
        filesCachePageStr = jsFilesCachePageStr + imgFilesCachePageStr;


        // Add entries for all required image files to SW
          ...
          filesSWPageStr = jsFilesSWPageStr;
        }
...
function updateLocalIndex(mcTmpDir) {
  console.log("updateLocalIndex", mcTmpDir);
  return new Promise((resolve, reject) => {
    const replace = require('replace-in-file');
...
module.exports = {
  ...
  prepI8DeployAssets3GC: prepI8DeployAssets3GC,
